...
//for port pada modal add vessel
    ...
    function setMinMaxDate() {
        ...
        const format9 = "YYYY-MM-DDTHH:mm:ss";
        day = moment(day).format(format9);
        week = moment(week).format(format9);

        $("#etBDry").attr("min",day);
        $("#etB").attr("min",day);
        $("#edit_etb").attr("min",day);
        $("#unreg_etb").attr("min",day);

        $("#etBDry").attr("max",week);
        $("#etB").attr("max",week);
        $("#edit_etb").attr("max",week);
        $("#unreg_etb").attr("max",week);

        $("#etDDry").attr("min",day);
        $("#etD").attr("min",day);
        $("#edit_etd").attr("min",day);
        $("#unreg_etd").attr("min",day);

        $("#etDDry").attr("max",week);
        $("#etD").attr("max",week);
        $("#edit_etd").attr("max",week);
        $("#unreg_etd").attr("max",week);
...
$('#btn_show_req_berth').on('click', function () {
    $('#div_request_berth').toggle();
    if($('#div_request_berth').is(":visible")) {
        $(this).text('hide request berth');
    }
})
