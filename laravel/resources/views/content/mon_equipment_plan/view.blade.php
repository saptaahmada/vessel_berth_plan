            ...
            <div class="col-md-12 form-horizontal">
              <div class="form-group">
                  ...
                  <div class="col-md-7">
                    ...
                    <button class="btn ripple-infinite btn-round btn-3d btn-warning" id="print_spk">
...
function getNodes(eq_type, key='CRANE') {
  $.ajax({
      url :  "{{url('MonEquipmentPlan/getNodes')}}",
      type : "post",
      dataType : "json",
      data : {
          "_token": "{{ csrf_token() }}",
...
function getVesBerth() {
  $.ajax({
      url :  "{{url('MonEquipmentPlan/getVesBerth')}}",
      type : "post",
      dataType : "json",
      data : {
          "_token": "{{ csrf_token() }}",
...
function getEq(key = "ALL") {
  $.ajax({
      url :  "{{url('MonEquipmentPlan/getEq')}}",
      type : "post",
      dataType : "json",
      data : {
          "_token": "{{ csrf_token() }}",
...
function getEqPlanHour() {
  $.ajax({
      url :  "{{url('MonEquipmentPlan/getEqPlanHour')}}",
      type : "post",
      dataType : "json",
      data : {
          "_token": "{{ csrf_token() }}",
...
function getEqTruckReady() {
  $.ajax({
      url :  "{{url('MonEquipmentPlan/getEqTruckReady')}}",
      type : "post",
      dataType : "json",
      data : {
          "_token": "{{ csrf_token() }}",
...
function getShiftMinMax() {
  $.ajax({
      url :  "{{url('MonEquipmentPlan/getShiftMinMax')}}",
      type : "post",
      dataType : "json",
      data : {
          "_token": "{{ csrf_token() }}",
...
$('#print_spk').on('click', function () {
  window.open("{{url('MonEquipmentPlan/print_spk')}}/"+$('#date').val(), '_blank');
})
...
function print() {
  ...
  window.open("{{url('MonEquipmentPlan/print')}}/"+arr_date[2]+'-'+arr_date[1]+'-'+arr_date[0]+'/'+planing_manager+'/'+berth_planner, '_blank');
}
...
function updateDataTruck(key = 'TRUCK') {
  ...
  if(m_allcell[key] != null) {
    ...
    m_data_truck = [arr_sts, arr_truck, arr_tt, arr_tb, arr_ht, arr_ctt, arr_tt_bds, arr_ht_bds, arr_req, arr_bmc_ht, arr_bst_lowbed,arr_total, arr_kekurangan];
    hot_truck.loadData(m_data_truck);
